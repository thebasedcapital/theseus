...
import torch
M1=Path(__file__).resolve().parent; REPO=M1.parent; EVAL=M1/'data'/'eval_wikitext.txt'; ROOT=M1/'work'/'fidelity'; CORPUS=ROOT/'corpus_8k.txt'; TAGS=('base','g4_perm','g5_c8','g5_c8_rep','prep_base'); NBYTES=8192
...
def main():
 ...
 raw=EVAL.read_bytes()[:NBYTES]; ROOT.mkdir(parents=True,exist_ok=True)
 if not CORPUS.exists() or CORPUS.read_bytes()!=raw: CORPUS.write_bytes(raw)
 results={}; cmds=[]; tstart=time.monotonic()
 for tag in [x.strip() for x in a.tags.split(',') if x.strip()]:
  d=common.REF_MODEL if tag=='base' else a.root/tag; d=Path(d).resolve(); tok=common.load_tokenizer(d); ids=torch.tensor(tok(CORPUS.read_text(),add_special_tokens=False)['input_ids'],dtype=torch.long)
